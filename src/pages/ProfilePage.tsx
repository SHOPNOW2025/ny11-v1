...
import { doc, updateDoc, addDoc, collection, increment, onSnapshot, getDoc } from "firebase/firestore";
import { 
    Wallet, 
    Settings, 
    LogOut, 
...
export default function ProfilePage({ user: loggedInUser, lang }: { user: UserProfile, lang: "ar" | "en" }) {
  ...
  const isOwnProfile = !uid || uid === loggedInUser.uid;
  const user = isOwnProfile ? loggedInUser : targetUser;
  ...
  useEffect(() => {
    if (!isOwnProfile && uid) {
      ...
      const unsub = onSnapshot(doc(db, "users", uid), (snap) => {
        if (snap.exists()) {
          setTargetUser({ uid: snap.id, ...snap.data() } as UserProfile);
        }
        setLoading(false);
      });
      return () => unsub();
  ...
  return (
    <div className="flex flex-col flex-1 pb-32">
      <header className="p-8 pt-12 flex flex-col items-center gap-4 bg-gradient-to-b from-primary/5 to-transparent">
        <label className={`relative ${isOwnProfile ? "cursor-pointer" : "cursor-default"} group`}>
          {isOwnProfile && <input type="file" className="hidden" accept="image/*" onChange={handleImageUpload} disabled={isUploading} />}
      ...
      {loading ? (
        <div className="flex-1 flex flex-col items-center justify-center py-20 opacity-20">
            ...
            <p className="text-[10px] font-black uppercase tracking-[0.3em]">{lang === "ar" ? "جاري التحميل..." : "Loading Profile..."}</p>
        </div>
      ) : user ? (
      <main className="p-4 space-y-6">
        {/* Wallet Card - Only show own wallet balance */}
        {isOwnProfile && (
            ...
            <div className="space-y-3">
              <div className="space-y-1">
                ...
                {isOwnProfile ? (
                    ...
                    <div className="w-full bg-white/5 rounded-2xl p-4 text-xs text-white/60 leading-relaxed font-arabic">
                        {user.bio_ar || (lang === "ar" ? "لا يوجد بايو متاح" : "No bio available")}
                    </div>
                )}
              </div>
              <div className="space-y-1">
                ...
                {isOwnProfile ? (
                    ...
                    <div className="w-full bg-white/5 rounded-2xl p-4 text-xs text-white/60 leading-relaxed">
                        {user.bio_en || (lang === "ar" ? "No bio available" : "No bio available")}
                    </div>
                )}
              </div>
              {isOwnProfile && (
        ...
        {/* Withdrawal Request Section for Experts - Only own */}
        {isOwnProfile && (user.role === "TRAINER" || user.role === "LAB_MANAGER") && (
          <WithdrawalSection user={user} lang={lang} />
        )}

        {/* Dashboards based on role - Only own */}
        {isOwnProfile && (
        ...
        {isOwnProfile ? (
            ...
            <button 
                onClick={() => navigate(-1)}
                className="w-full py-4 glass border border-white/10 rounded-2xl text-white/60 font-bold flex items-center justify-center gap-2 hover:bg-white/5 transition-colors"
            >
                <ChevronLeft size={18} />
        ...
        <div className="flex-1 flex flex-col items-center justify-center py-20 opacity-20">
            <h2 className="text-sm font-black uppercase tracking-widest">{lang === "ar" ? "المستخدم غير موجود" : "User Not Found"}</h2>
            <button onClick={() => navigate(-1)} className="mt-4 text-xs text-primary underline">{lang === "ar" ? "عودة" : "Back"}</button>
        </div>
      )}
    </div>
  );
